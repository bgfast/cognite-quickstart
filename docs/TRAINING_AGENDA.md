...
| **0:35–0:55** | NEAT (hw-neat) | • Purpose: Excel-based data model → CDF via NEAT and toolkit<br>• Open `modules/hw-neat/data_models/HWNeatBasic.xlsx` — source of truth for space, container, view<br>• Run `python generate_edm_yaml_files.py` from `modules/hw-neat` to generate YAML under `data_models/data_models/`<br>• Redeploy if needed: `cdf build --env=hw-neat` and `cdf deploy --env=hw-neat`<br>• In CDF: open the **hw-neat** Streamlit app; create/read/update/delete instances (CRUD) to see the NEAT model in action |
